...
//preprare variables for the quaries
$username =
...
//256bits twice od md5 = 64 characters u heksadecimalen sistem 

//$password = md5($password);
//128 bits e ama avtomaski u hecidecimalen - > 32 karaktersi znaci
...
//if email exits in the users table print error
...
$activationKey = bin2hex(openssl_random_pseudo_bytes(16));
    ...
    //(2*2*2*2)*2*2*2*2..*2 base two (128 dvojke imamo)
    //base sixteen (heksadecimalen) neli toa e 2*2*2*2 = 16 * 16 *16 .. *16 ( 128/4 = 32)
    //32 characters in the user table
...
$result = mysqli_query($link, $sql);
if(!$result){
     echo '<div class="alert alert-danger">There was an error insertin the users details in the database!</div>'; 
     
     exit;
}


/*Sent the user an email with a link to activate.php with their email and activaction code*/
$message="Please click on this link to activate your account:\n\n";
$message .= "http://imakesites.host20.uk/OnlineNotesApp/activate.php?email=" . urldecode($email) . "&key=$activationKey";

if(mail($email, 'Confirm your Registration', $message, 'From:'.'simonabushevska99@gmail.com')){
