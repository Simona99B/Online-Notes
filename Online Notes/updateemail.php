...
if($count>0){
    echo "<div class='alert alert-danger'>There is already user that is registered with that email. Please choose another one!</div>";
    exit;
}
...
$result = mysqli_query($link,$sql);
if(!$result){
    echo "<div class='alert alert-danger'>There was an error inserting the user email in the database</div>";
}else{
    ...
    $message .= "http://imakesites.host20.uk/OnlineNotesApp/activatenewemail.php?email=" . urldecode($email) . "&newemail=" . urlencode($newemail) . "&key=$activationKey";

    if(mail($newemail, 'Email Update for your Online Notes App', $message, 'From:'.'simonabushevska99@gmail.com')){
